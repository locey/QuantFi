...
import {
    ...
    HLConstants,
    HLConversions
} from "@hyper-evm-lib/src/CoreWriterLib.sol";
...
contract BridgingExample {
     ...
     */
    function bridgeToCoreByAddress(
     ...
     * @notice Bridges tokens to core and then sends them to another address
     */
    function bridgeToCoreAndSendById(
        uint64 tokenId,
        uint256 evmAmount,
        address recipient
    ) external payable {
    ...
    function getTokenIndex(
        address tokenAddress
    ) external view returns (uint64 tokenId) {
