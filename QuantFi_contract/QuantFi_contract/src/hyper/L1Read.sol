...
contract L1Read {
    ...
    address constant SPOT_INFO_PRECOMPILE_ADDRESS =
    ...
    address constant TOKEN_SUPPLY_PRECOMPILE_ADDRESS =
        0x000000000000000000000000000000000000080D;
    address constant BBO_PRECOMPILE_ADDRESS =
    ...
    function spotBalance(
        address user,
        uint64 token
    ) public view returns (SpotBalance memory) {
        bool success;
        bytes memory result;
    ...
    function withdrawable(
        address user
    ) public view returns (Withdrawable memory) {
        bool success;
        bytes memory result;
    ...
    function delegatorSummary(
        address user
    ) public view returns (DelegatorSummary memory) {
        bool success;
        bytes memory result;
    ...
    function l1BlockNumber() public view returns (uint64) {
        bool success;
        bytes memory result;
        (success, result) = L1_BLOCK_NUMBER_PRECOMPILE_ADDRESS.staticcall(
            abi.encode()
        );
    ...
    function perpAssetInfo(
        uint32 perp
    ) public view returns (PerpAssetInfo memory) {
        bool success;
        bytes memory result;
    ...
    function spotInfo(uint32 spot) public view returns (SpotInfo memory) {
        bool success;
        bytes memory result;
        (success, result) = SPOT_INFO_PRECOMPILE_ADDRESS.staticcall(
            abi.encode(spot)
        );
    ...
    function tokenSupply(
        uint32 token
    ) public view returns (TokenSupply memory) {
        bool success;
        bytes memory result;
        (success, result) = TOKEN_SUPPLY_PRECOMPILE_ADDRESS.staticcall(
            abi.encode(token)
        );
    ...
    function bbo(uint32 asset) public view returns (Bbo memory) {
        bool success;
        bytes memory result;
        (success, result) = BBO_PRECOMPILE_ADDRESS.staticcall(
            abi.encode(asset)
        );
    ...
    function accountMarginSummary(
        ...
        address user
    ) public view returns (AccountMarginSummary memory) {
        bool success;
        bytes memory result;
        (success, result) = ACCOUNT_MARGIN_SUMMARY_PRECOMPILE_ADDRESS
            .staticcall(abi.encode(perp_dex_index, user));
        require(success, "Account margin summary precompile call failed");
        return abi.decode(result, (AccountMarginSummary));
    }

    function coreUserExists(
        address user
    ) public view returns (CoreUserExists memory) {
        bool success;
        bytes memory result;
        (success, result) = CORE_USER_EXISTS_PRECOMPILE_ADDRESS.staticcall(
            abi.encode(user)
        );
